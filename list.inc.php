		...
		if($result && mysql_num_rows($result) > 0 ) {
			...
			while(list($num, $kaitongleixin,$gongdan, $user_name, $user_num, $user_address, $onu_type,$onu_mac,$olt,$pon) = mysql_fetch_row($result)) {
			  ...
			  //echo '<td>'.date("Y-m-d",$ptime).'</td>';
				...
				echo '<td>'.$olt.'</td>';
				echo '<td>'.$pon.'</td>';
				echo '<td><a href="index.php?action=mod&num='.$num.'">修改</a>/<a onclick="return confirm(\'你确定要删除号码'.$user_num.'吗?\')" href="index.php?action=del&num='.$num.'&user_num='.$user_num.'">删除</a></td>';
				echo '</tr>';
			}
